---
title: 使用 MNIST 旋转角度预测理解Mixture of Experts中的合作与竞争损失
date: 2026-05-09
category: 深度学习
...
最近阅读了 Jacobs、Jordan、Nowlan 和 Hinton 在 1991 年发表的论文 *Adaptive Mixtures of Local Experts*。这篇论文应该是最早探讨MOE架构的文章，其中讨论了一个很有启发性的想法：假设一个任务天然可以分解成多个子任务，那么与其训练一个单一网络处理所有样本，不如训练多个 local experts，再由一个 gating network 决定每个样本应该交给哪些 expert。

这篇论文对 MOE 专家的训练提出了多种损失函数，代表了不同的训练目标。不同损失函数会改变 experts 之间的关系：有的损失鼓励多个 expert 合作完成预测，有的损失鼓励 expert 之间竞争，让 gating network 对每个样本做更明确的分配。

为了对这个机制形成直观理解，笔者设计了一个小规模实验：使用 `torchvision.datasets.MNIST` 构造 MNIST 角度回归任务，并比较论文中提到的三类 MOE 损失。实验目标不是在 MNIST 上追求最优性能，而是观察三种损失是否真的会导致不同的专家协作和竞争行为（至于为什么选这个实验，是因为既想使用集成好的数据集简化代码，也想以最快的速度完成实验）。
...
没有直接做 MNIST 分类，而是把 MNIST 改造成一个图像回归任务：输入为旋转后的数字图像，输出为旋转角度。
...
第一轮实验后，一个自然问题是：竞争式损失效果不如合作式损失，是因为数据量太小，还是因为任务和模型不适配？因此又额外做了三个扩展消融实验。
...
固定训练样本数为 `12000`，固定随机种子为 `7`，将 expert 从线性层改为带一个隐藏层的 `MLP-32`。
...
另一个值得注意的现象是，`MLP-32` 下竞争式损失的 gate entropy 进一步降低到接近 0，但 Expert Usage 显示几乎所有样本都被路由到单个 expert。这说明提高容量虽然改善了误差，却也加重了 expert collapse：模型更容易让一个较强 expert 吸收全部任务，也就是说尽管论文中的负对数混合似然损失理论上可以是MOE既有竞争，又能相互兼顾，但是在这个任务的实验中表现还是差强人意。

## E：多随机种子稳定性

最后在随机种子 `7/11/19` 下重复训练，观察结论是否稳定。
...
第三，任务结构很重要。论文原始实验是低维元音识别，局部线性 expert 很容易解释，也容易形成清楚的任务分解。Rotated MNIST 是高维图像任务，即便人为设置了角度组，输入空间中的分工结构也不如二维任务直观。因此，这个任务能帮助理解 MOE 的训练行为，但并不一定能展示竞争式 MOE 的最好效果。因此关于现在LLM或ViT如何设计和使用MOE还需进一步的学习。
...
对笔者来说，这个实验最大的价值是区分了两个容易混淆的问题：MOE 是否形成了专家竞争，以及这种竞争是否提升了预测性能。前者在实验中已经比较清楚地出现了；后者则依赖任务结构、expert 容量和训练策略，并不是损失函数本身能单独保证的。
